...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitDomain(Domain domain) {
      ...
      if (!domain.getGroups().isEmpty()) {
         for (Group group : domain.getGroups().values().toArray(new Group[0])) {
   ...
   public void visitDomainGroup(DomainGroup domainGroup) {
      startTag(ENTITY_DOMAIN_GROUP, null);

      if (!domainGroup.getDomains().isEmpty()) {
         for (Domain domain : domainGroup.getDomains().values().toArray(new Domain[0])) {
            domain.accept(m_visitor);
         }
      }

      endTag(ENTITY_DOMAIN_GROUP);
   }
   ...
   public void visitGroup(Group group) {
      ...
      if (!group.getIps().isEmpty()) {
         for (String ip : group.getIps().toArray(new String[0])) {
