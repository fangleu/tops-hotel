...
import com.dianping.cat.home.activity.IEntity;
...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitActivity(Activity activity) {
      startTag(ENTITY_ACTIVITY, true, null, ATTR_DOMAIN, activity.getDomain(), ATTR_TYPE, activity.getType(), ATTR_NAME, activity.getName(), ATTR_TITLE, activity.getTitle());
   }

   @Override
   public void visitActivityConfig(ActivityConfig activityConfig) {
      startTag(ENTITY_ACTIVITY_CONFIG, null);

      if (!activityConfig.getActivities().isEmpty()) {
         for (Activity activity : activityConfig.getActivities().toArray(new Activity[0])) {
            activity.accept(m_visitor);
         }
      }

      endTag(ENTITY_ACTIVITY_CONFIG);
   }
}
