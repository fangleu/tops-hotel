...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitProductLine(ProductLine productLine) {
      startTag(ENTITY_PRODUCTLINE, true, null, ATTR_ID, productLine.getId(), ATTR_COLINSIDE, productLine.getColInside());
   }

   @Override
   public void visitTopoGraphFormatConfig(TopoGraphFormatConfig topoGraphFormatConfig) {
      startTag(ENTITY_TOPOGRAPH_FORMAT_CONFIG, null);

      if (!topoGraphFormatConfig.getProductLines().isEmpty()) {
         for (ProductLine productLine : topoGraphFormatConfig.getProductLines().toArray(new ProductLine[0])) {
            productLine.accept(m_visitor);
         }
      }

      endTag(ENTITY_TOPOGRAPH_FORMAT_CONFIG);
   }
}
