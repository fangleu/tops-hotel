...
public class DefaultJsonParser {
         ...
         if (parent instanceof BugReport) {
            if (ENTITY_DOMAINS.equals(tag)) {
               ...
               if (ENTITY_DOMAINS.equals(parentTag)) {
                  ...
                  m_linker.onDomain((BugReport) parent, domains);
                  m_objs.push(domains);
               } else {
            ...
            if (ENTITY_EXCEPTION_ITEMS.equals(tag)) {
               m_objs.push(parent);
            } else {
               String parentTag = m_tags.size() >= 2 ? m_tags.get(m_tags.size() - 2) : null;

               if (ENTITY_EXCEPTION_ITEMS.equals(parentTag)) {
                  ExceptionItem exceptionItems = new ExceptionItem();

                  m_linker.onExceptionItem((Domain) parent, exceptionItems);
                  m_objs.push(exceptionItems);
               } else {
                  throw new RuntimeException(String.format("Unknown tag(%s) found at %s!", tag, m_tags));
               }
            }
   ...
   protected void onValue(String value) {
      ...
      if (parent instanceof BugReport) {
         parseForBugReport((BugReport) parent, tag, value);
      } else if (parent instanceof Domain) {
         parseForDomain((Domain) parent, tag, value);
      } else if (parent instanceof ExceptionItem) {
         parseForExceptionItem((ExceptionItem) parent, tag, value);
      } else {
   ...
   public void parseForBugReport(BugReport bugReport, String tag, String value) {
      if (ENTITY_DOMAINS.equals(tag)) {
         // do nothing here
      } else if (ATTR_STARTTIME.equals(tag)) {
         bugReport.setStartTime(toDate(value, "yyyy-MM-dd HH:mm:ss"));
      } else if (ATTR_DOMAIN.equals(tag)) {
         bugReport.setDomain(value);
      } else if (ATTR_ENDTIME.equals(tag)) {
         bugReport.setEndTime(toDate(value, "yyyy-MM-dd HH:mm:ss"));
      } else {
         throw new RuntimeException(String.format("Unknown tag(%s) of %s under %s!", tag, bugReport, m_tags));
      }
   }
   ...
   public void parseForDomain(Domain domain, String tag, String value) {
      if (ENTITY_EXCEPTION_ITEMS.equals(tag)) {
         ...
         domain.setId(value);
      } else if (ELEMENT_PROBLEM_URL.equals(tag)) {
         domain.setProblemUrl(value);
      } else if (ATTR_DEPARTMENT.equals(tag)) {
         domain.setDepartment(value);
      } else if (ATTR_PRODUCTLINE.equals(tag)) {
         domain.setProductLine(value);
      } else if (ELEMENT_EXCPETION.equals(tag)) {
         domain.setExcpetion(value);
      } else {
         throw new RuntimeException(String.format("Unknown tag(%s) of %s under %s!", tag, domain, m_tags));
      }
   }

   public void parseForExceptionItem(ExceptionItem exceptionItem, String tag, String value) {
      if (ATTR_ID.equals(tag)) {
         exceptionItem.setId(value);
      } else if (ATTR_COUNT.equals(tag)) {
         exceptionItem.setCount(convert(Integer.class, value, 0));
      } else if (ELEMENT_MESSAGES.equals(tag)) {
         exceptionItem.addMessage(value);
      } else {
         throw new RuntimeException(String.format("Unknown tag(%s) of %s under %s!", tag, exceptionItem, m_tags));
      }
   }
