...
public class DefaultXmlBuilder implements IVisitor {
   ...
   public void visitAllConfig(AllConfig allConfig) {
      ...
      if (!allConfig.getReports().isEmpty()) {
         for (Report report : allConfig.getReports().values().toArray(new Report[0])) {
   ...
   public void visitReport(Report report) {
      ...
      if (!report.getTypes().isEmpty()) {
         for (Type type : report.getTypes().values().toArray(new Type[0])) {
   ...
   public void visitType(Type type) {
      ...
      if (!type.getNameList().isEmpty()) {
         for (Name name : type.getNameList().toArray(new Name[0])) {
