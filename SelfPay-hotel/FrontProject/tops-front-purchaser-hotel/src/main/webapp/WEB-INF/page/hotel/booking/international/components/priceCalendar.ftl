<#--	价格日历		-->
<#macro priceCalendar id criteria itemPrice bookingClassDailyRates=[]>
<div id="${(id)!}" class="layerGrayBlock" style="z-index:100;display:none;font-size: 12px; position:absolute;">
	<#if (criteria.validDates)??>
	<#assign validDates=criteria.validDates />
	<div class="layerGrayContainer">
		...
		<table class="abroadhotelSalePriceTbl">
			<thead>
			<tr>
				<th class="first" style="width:60px;"><b>第${week}周</b></th>
				<#list 0..6 as i>
				<#assign day = validDates[i]?date("yyyy-MM-dd")?string("EEE") />
				<th class="<#if day="星期六"|| day="星期日">weekend</#if><#if i=6> last</#if>">
					周${day?substring(2,3)}
				</th>
				</#list>
			</tr>
